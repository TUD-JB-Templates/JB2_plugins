...
const intermezzoTransform = {
  ...
  plugin: (opts, utils) => (tree) => {
    ...
    rootChildren.forEach((node) => {
      ...
      const hasIntermezzoClass =
        (typeof classes === "string" && classes.includes("admonition-intermezzo")) ||
        (Array.isArray(classes) && classes.includes("admonition-intermezzo"));

      if (!hasIntermezzoClass) return;
      ...
      const label = node.label || `Intermezzo`;
      labelMap.set(label, [originalTitle]);

      // Re-title it to "Intermezzo: <title>"
      // Replace the children of the title node with a single text node
      if (titleNode) {
        titleNode.children = [{ type: "text", value: `Intermezzo: ${originalTitle}` }];
      } else {
        // If for some reason there is no title node, prepend one
        node.children = [
          {
            type: "admonitionTitle",
            children: [{ type: "text", value: `Intermezzo: ${originalTitle || ""}` }]
          },
          ...(node.children || [])
        ];
      }
      ...
      if (Array.isArray(node.classes)) {
        ...
        node.class = node.class
          .split(/\s+/)
          .filter(c => c && c !== "admonition-intermezzo")
          .join(" ");
      }
    });
  },
